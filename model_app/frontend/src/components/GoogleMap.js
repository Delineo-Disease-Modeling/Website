...
const GoogleMap = ({ center = { lat: 32, lng: -86 }, zoom = 10 }) => {
  ...
  return (
    <Fragment>
      ...
      {options.map(option => (<Checkbox key={option} isSelected={mapState.checkboxes[option]}
        onCheckboxChange={handleCheckboxChange} label={option} />))}
      <Button onClick={handleButtonChange}>Draw Polygon!</Button>
      {mapApiLoaded && <Polygon map={mapInstance} mapApi={mapApi} editable={mapState.editPolygon} />}
