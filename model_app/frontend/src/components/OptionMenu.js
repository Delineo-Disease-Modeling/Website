...
const OptionMenu = () => {
  ...
  const deleteItem = (pos, policy) => {
    setMenuState({
      policyComponent: menuState.policyComponent.filter(
  ...
  const save = () => {
    if (menuState.duration !== "" && menuState.selectedPolicy !== "Policy") {
      ...
      setMenuState({
        policyComponent: [...menuState.policyComponent, newPolicy],
      });
    ...
    <div>
      ...
      <div align="left" className={menuState.hidden ? "hidden" : ""}>
        <div className="row">
          <div className="col">
            <Dropdown
              isOpen={menuState.dropdownOpen}
              toggle={toggle}
              style={{ marginLeft: "10px" }}
            >
              <DropdownToggle caret>
                {" "}
                {menuState.selectedPolicy}{" "}
              </DropdownToggle>
              <DropdownMenu>
                {menuState.policyList.map((item) => {
                  return (
                    <DropdownItem
                      key={item}
                      onClick={() => {
                        setMenuState({ selectedPolicy: item });
                      }}
                    >
                      {item}
                    </DropdownItem>
          ...
          <div className="col">
            <label htmlFor="usr" style={{ color: "white" }}>
              Duration (in days):{" "}
            </label>
            &nbsp; &nbsp;
            <input
              ...
              onChange={(event) =>
                setMenuState({ duration: event.target.value })
              }
            ></input>
          </div>
          <div align="bottom" className="col4">
            <button className="buttonSave" onClick={save}>
              Save
            </button>
          </div>
