...
class Polygon extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.place !== prevProps.place) {
      ...
      axios
        .get(
          `https://nominatim.openstreetmap.org/search?q=${place.formatted_address}&format=json&addressdetails=1&limit=1&polygon_geojson=1`
        )
        .then((res) => {
          ...
          if (data["geojson"]) {
            ...
            axios
              .get(
                `http://polygons.openstreetmap.fr/get_geojson.py?id=${data["osm_id"]}&params=0`
              )
              .then((results) => {
