...
test("'Fullstack' item on Meet the Team nav bar works", () => {
  ...
  test("'Simulation' item on Meet the Team nav bar works", () => {
    ...
    const button = getAllByRole('button').filter(button => button.textContent === 'Simulation');

    // Simulate clicking Simulation button
    fireEvent.click(button[0]);
  
    // Make assertion
    getByText("The simulation team is responsible for the implementation of the simulation that lies at the heart of the Delineo project. Team members focus on programming the various modules, algorithms, and statistical drivers that are utilized in our models.");
  ...
  test("'Machine Learning' item on Meet the Team nav bar works", () => {
    ...
    const button = getAllByRole('button').filter(button => button.textContent === 'Machine Learning');
  ...
  test("'Information Support' item on Meet the Team nav bar works", () => {
    ...
    const button = getAllByRole('button').filter(button => button.textContent === 'Information Support');

    // Simulate clicking Information Support button
    fireEvent.click(button[0]);
  
    // Make assertion
    getByText("The information support team is responsible for gathering, summarizing, and disseminating research articles/papers that prove helpful for informing the development of Delineo's model.");
  ...
  test("'All' item on Meet the Team nav bar works", () => {
    ...
    //Retrieve Fullstack button
    const fButton = getAllByRole('button').filter(button => button.textContent === 'Fullstack');
    ...
    //Retrieve All button
    const aButton = getAllByRole('button').filter(button => button.textContent === 'All');
